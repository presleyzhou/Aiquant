...
| **Scheduled recheck & admin** | Daily GitHub Action re-grades every listed / synced factor into KV (`/api/admin/recheck`); library and seller panels show server-side health badges (`/api/factors/health`). Minimal operator page (`?admin=1`, `X-Admin-Token`): overview, withdrawals (pay / reject with refund), orders, listings, manual recheck. Factor library → Pipeline hand-off shares one factor set across both engines |
