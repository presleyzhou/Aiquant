...
| **Backtesting** | Next-bar-open fills, commission + slippage both sides, buy-and-hold benchmark always shown; SMA/EMA cross, RSI reversion, Kronos-signal strategies; walk-forward validation Risk controls: stop loss, take profit, trailing stop, vol-targeted sizing and max position (decided on the previous close, filled next open; exits by reason reported). |
...
| **AI Factor Mining** | Chain-of-Alpha-style loop: Claude proposes factor expressions in a safe DSL (hand-rolled parser, never `eval`), a deterministic evaluator scores daily rank IC with an untouched holdout, and directive feedback steers each next round; cross-session memory, decay monitoring, cross-market transfer tests, multi-factor composites Hourly `crypto_1h` market (40 coins × ~90 days of 1h bars, Binance → Yahoo → CoinGecko chain, 8760 periods/yr); regime heatmap (sign-aligned IC per quarter, per week for hourly); correlation-clustered factor families with equal-per-family composite weighting; listed factors carry a live-record badge (days listed, recent IC, grades, re-checked daily). |
| **End-to-End Pipeline** | Universe (built-in 60 US / 24 crypto, or a custom 8–40 ticker list with 3y/5y history; crypto bars from Binance public klines with CoinGecko filling unlisted coins, US bars from AkShare/Sina when installed, Yahoo as fallback) → multi-factor signal (expanding-window IC weights, so every day is out-of-sample; IC-decay curve, quintile spreads) → portfolio construction (equal / score / inverse-vol / min-variance / risk parity / **HRP** / **Grinold mean-variance** on a Ledoit-Wolf-shrunk covariance; caps, shrink-to-1/N, hold buffer, Gârleanu-Pedersen partial adjustment, vol targeting) → cost-aware backtest with price drift between rebalances, in-sample vs holdout, monthly heatmap, seven-scheme comparison with a Ledoit-Wolf bootstrap test of each scheme against 1/N → **overfitting check** (Probabilistic & Deflated Sharpe with an honest trial count, Sharpe t-stat vs the Harvey-Liu-Zhu hurdle, minimum track record length, breakeven cost, rolling half-year hit rate, a 3×3 parameter-sensitivity grid with spike detection) → risk & attribution (drawdowns, contributors, effective N, beta/TE/IR, capture ratios, CVaR, regime table, Brinson sector attribution, square-root-impact capacity curve with breakeven AUM) → target weights, one-click deploy to paper trading, a rebalance order ticket (NAV + current shares → sell-first whole-share trade list with costs) and an optional AI investment-committee memo. Pure numpy/pandas |
| **Scheduled ops & admin** | One daily GitHub Action (`ops.yml`) calls `POST /api/admin/ops`: warm the shared panels → re-grade every listed / synced factor into KV → run the deployment monitor, looping until no accounts remain; the run report and a 7-day per-provider data-source health table (calls, Yahoo fallback rate, avg download time) show in the admin console. `ADMIN_TOKEN_NEXT` lets the token rotate with two valid values. A nightly `live-check.yml` downloads the real US / crypto panels, runs the default pipeline and builds a ticket on a networked runner, opening a `live-check` issue on failure. Pipeline runs are cached in KV by config hash + panel date (`cached: true`). V4 adds CPCV (6 groups, k=2 → 5 stitched out-of-sample paths, purged + embargoed; path-Sharpe distribution), an L1 turnover penalty in the mean-variance objective (Gârleanu-Pedersen aim portfolio, exact prox), a dollar-neutral long-short mode with borrow cost (margin/locate not modelled; tickets stay long-only), a survivorship-bias notice on every universe and optional point-in-time fundamentals via `FMP_API_KEY` (DSL fields `mcap pe pb roe ep bp`, published after `FUNDAMENTALS_LAG_DAYS`); library and seller panels show server-side health badges (`/api/factors/health`). Minimal operator page (`?admin=1`, `X-Admin-Token`): overview, withdrawals (pay / reject with refund), orders, listings, manual recheck. Factor library → Pipeline hand-off shares one factor set across both engines Admin tiers: `ADMIN_READONLY_TOKEN` views everything but cannot settle / resolve / trigger; `ADMIN_EMAILS` makes listed Supabase accounts admins on sign-in. Buyers may open a refund dispute within `REFUND_WINDOW_DAYS` (account or entitlement token proves ownership); the console refunds (token revoked, order marked, wallet purchases re-credited, Stripe refunded at the provider, Coinbase credited to the wallet or flagged manual, seller cut clawed back) or rejects, and an append-only audit log records settlements, refunds, withdrawals, listings, claims and admin actions. `keepalive.yml` pings the Vercel API every 10 minutes; `tests/test_api_contract.py` checks every `/api/…` path + method in `api.ts` and the Playwright mocks against the FastAPI route table. |
| **Accounts (optional)** | Supabase Auth magic-link sign-in; the backend verifies bearer tokens via `/auth/v1/user`. Signed-in users get an additive, key-aware cloud merge of factor library, lessons, paper deployments, alerts, purchases and watchlists; wallet and listings belong to the account, with a one-click claim of the browser identity. Env: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `ADMIN_TOKEN` Claiming also moves orders, withdrawal requests and disputes to the account, so refunds and rejected withdrawals land in the wallet the user can see. On marketplace load the browser verifies its stored entitlement tokens (`POST /api/marketplace/entitlements/verify`); refunded purchases are dropped, the card returns to purchasable and a synced tombstone keeps the cloud merge from resurrecting them. `/api/version` is the real commit SHA on both deployments, so the integrations page can tell a stale Space from a current one. |
...
| **Daily monitor** | A scheduled job replays every signed-in user's deployments after the close, flags drawdown breaches, edge decay, rebalance-due (target holdings changed), stale data and replay errors, stores the report for the paper page and pushes NEW alerts once to a user-configured Slack / Discord / Telegram webhook (https, public hosts only) |
