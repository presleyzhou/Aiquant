...
**系统协同补漏。** 四处模块之间真正断掉的交接被接上：① 账号合并（`/api/account/claim`）现在连同订单、提现申请与争议记录一起从浏览器身份转到登录账号，此前只搬钱包与上架 —— 否则站长驳回一笔合并前的提现会把钱退回已清空的浏览器钱包，买家也无法对合并前的购买提出争议；② 退款会传导到买家的浏览器：市场页加载时把本地持有的权益凭证批量送到 `POST /api/marketplace/entitlements/verify`，被撤销的购买从本地移除、卡片恢复可购买并提示，同时写入可同步的「已撤销」墓碑（`aiquant.purchases.revoked`），云端合并不会把失效授权带回来；③ 小时级因子（`crypto_1h`）纳入每日重检与一键预热，上架后同样获得「上线后成绩」徽标，因子库的「→ Pipeline」在只有小时级因子时置灰并说明原因（流水线只跑日线）；④ `/api/version` 改为真实构建指纹（Vercel 的 `VERCEL_GIT_COMMIT_SHA`，HF Space 读取 `.git/HEAD`），后台「集成状态自检」据此能真正判断 Space 是否落后于主站；`/api/health` 同时返回版本与持久化模式，供每 10 分钟的保活探测顺带核对。
