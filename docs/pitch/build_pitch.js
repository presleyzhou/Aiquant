...
(async () => {
  ...
  // ================================================================ 1 cover
  {
    ...
    notes(s, "【0:00–0:40】开场。一句话：AIQUANT 是一个已经上线运行的 AI 投研网站——它让 AI 提出投资因子，用一套严格的样本外检验把噱头筛掉，再把通过检验的信号变成交易单，并允许研究者把它挂到市场上卖。今天 10 分钟讲三件事：为什么现在做、我们做了什么、为什么值得投。\n提示：全程强调「已上线、可演示」，不要讲开发过程中的困难。");
  }
  ...
  // ================================================================ 2 problem
  {
    ...
    footer(s, 2);
    notes(s, "【0:40–1:40】痛点。核心一句：过去投研的瓶颈是「产生想法」，现在 AI 把这一步的成本打到零，瓶颈转移到了「可信的验证」和「上线后的持续监控」。三个断点：验证慢、过拟合、没人盯。\n落点：谁能把「AI 生成 → 严格验证 → 持续监控」做成一条自动化流水线，谁就掌握了 AI 时代投研的关键环节。");
  }
  ...
  // ================================================================ 3 solution: input → process → output
  {
    ...
    footer(s, 3, true);
    notes(s, "【1:40–2:40】解决方案总览。强调这不是概念图，而是网站上真实存在的四个环节，等一下现场就演示：输入一句假设，几分钟后拿到一个带评级的因子和可执行的目标持仓。\n关键词：闭环（Loop Engineering）、样本外、可交易、可销售。");
  }
  ...
  // ================================================================ 4 product overview
  {
    ...
    footer(s, 4);
    notes(s, "【2:40–3:30】产品全景。左边是真实首页截图。四个模块一句话带过，重点放在「全部接在同一套数据与引擎上」：市场里买的策略一键进回测，因子库里的因子一键进流水线，流水线的结果一键部署成模拟持仓。这种「一体化」是我们与单点工具的差别。");
  }
  ...
  // ================================================================ 5 core tech 1: loop engineering
  {
    ...
    footer(s, 5, true);
    notes(s, "【3:30–4:30】技术一。「Loop Engineering」是我们的方法论名词：AI 不是一次性生成，而是在确定性的评估器和规则约束下反复迭代，每一轮拒绝的理由都会变成下一轮的指令。这与 Chain-of-Alpha 等论文思路一致，但我们把它做成了产品，并加了第二台不依赖 LLM 的遗传进化引擎做交叉验证。");
  }
  ...
  // ================================================================ 6 core tech 2: gates
  {
    ...
    footer(s, 6);
    notes(s, "【4:30–5:30】技术二。这一页是我们与竞品最大的区别：AI 生成因子已经不稀缺，稀缺的是把过拟合、不可交易、随机撞中的因子拦下来。五道门禁 + 报告卡 + 上线后成绩，构成了我们的「信任层」。举例：一个回测 IC 很高但 Top-5 每天换 60% 仓位的因子，扣成本就是负的——门禁直接拦下，并把理由反馈给 AI 下一轮避免。");
  }
  ...
  // ================================================================ 7 end to end: pipeline + paper + monitor
  {
    ...
    footer(s, 7, true);
    notes(s, "【5:30–6:15】闭环的后半段。研究成果不停留在报告：流水线给出目标持仓和交易单；模拟持仓从部署那天起样本外记录；服务器每天替用户复检并推送告警。对投资人的含义：用户留存来自「每天都要回来看」，而不是一次性工具。");
  }
  ...
  // ================================================================ 9 business model
  {
    ...
    footer(s, 9, true);
    notes(s, "【6:45–7:30】商业模式。第一层已经在线上跑通（演示模式下可完整走完购买流程；接入真实支付通道只是配置密钥）。第二层订阅是近期重点；第三层机构 API 是中期方向。飞轮：越多人挖掘，评估器与记忆越强，越多高质量因子进入市场，越多买家——研究工具本身就是获客漏斗。\n注意：不要提供具体收入预测数字，被问到时用「首年目标：上架研究者 500 人、付费转化 5%」这类目标口径回答。");
  }
  ...
  // ================================================================ 10 market
  {
    ...
    footer(s, 10);
    notes(s, "【7:30–8:00】市场。三组数字都是公开口径（路演前请再核对最新数值）：中登公司 2023 年 A 股投资者数突破 2.2 亿；全球加密持有者 5 亿以上为行业机构估算；Bloomberg 终端约 2.5 万美元/年/席位。我们的判断：AI 让研究能力民主化，但专业级的验证与监控工具仍是空白，这就是我们的位置。");
  }
  ...
  // ================================================================ 11 moat
  {
    ...
    footer(s, 11, true);
    notes(s, "【8:00–8:40】壁垒。投资人会问「大模型公司或券商为什么不做」：大模型公司不做垂直的验证与交易闭环；券商受合规与组织约束不做开放市场。我们的护城河在于把学术界成熟的方法论产品化，并在市场层建立了信任机制——这两者叠加需要时间和真实运行数据。工程质量数字是真实的 CI 数据。");
  }
  ...
  // ================================================================ 12 traction + roadmap
  {
    ...
    footer(s, 12);
    notes(s, "【8:40–9:10】进展。四个数字都是真实的：12 个模块、118+40 标的宇宙、80 多个 API 端点、311 个自动化测试。路线图重点讲 Q4：开通真实支付与首批上架研究者，是从「产品」到「生意」的转折点。");
  }
  ...
  // ================================================================ 13 ask
  {
    ...
    footer(s, 13, true);
    notes(s, "【9:10–9:40】融资需求。金额与比例是建议方案，路演前请按自身实际调整（导师会追问估值与出让比例，建议准备：天使轮出让 10–15%）。用途重点：研发与数据占大头，因为 A 股接入与移动端是下一阶段增长引擎；合规预算显示我们对支付与资金托管的重视。18 个月目标全部是可验证的经营指标。");
  }
  ...
  // ================================================================ 14 close
  {
    ...
    footer(s, 14, true);
    notes(s, "【9:40–10:00】收尾。回到开头那句话，然后明确给出「邀请」。留 10 秒给评委记网址。随后进入 10 分钟导师互动——常见问题的答法见讲稿附录。");
  }
